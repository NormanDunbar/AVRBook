...
Click the \inline{prepare} button. After a very brief pause, you should notice that the project window has changed and a new folder named \path{arduino-blink} appears there. Open up the \path{src} folder and double-click on the file \path{Blink.cpp} which you should find there.
...
Looks familiar? It should do, it's simply the Arduino Blink example code, with one minor difference, the inclusion of the \path{Arduino.h} header file.
...
Now select a directory to save the code in. My choice is \path{/home/norman/SourceCode/PlatformIO/AVRBlink} you might want to choose something different, especially if you are on Windows!

\begin{note}
You can obtain the source code, but not the \indexasis{PlatformIO} project files etc, for all the examples in this book from \href{https://github.com/NormanDunbar/AVRBook}{GitHub} - look for the folder named \path{AVRCode}. The code files will be found in the same part and/or chapter as the book.
\end{note}
...
\begin{itemize}
\item
  A directory named \path{lib} and a file, \path{readme.txt} within.   The README file has all the information you will need if you intend to   write some libraries for this particular project. The source code for   the libraries should be kept in this directory - with a separate directory for each separate library.
\item
  A directory named \path{src}. This is where you will create your own   project's source code.
\item
  A file named \path{.gitignore}. This hidden file on Linux tells git, if git is in use for   version control, to ignore a number of files that are not required in   normal circumstances, usually files that can be recreated at will.
\item
  A file named \path{.travis.yml}. This hidden file on Linux is a YAML source file that   tells the Travis `Continuous Integration' system what to do whenever   something in the project changes. We will not be using this feature.
\item
  A file named \path{platformio.ini}. This file tells PlatformIO how the   project will be built and uploaded to the microcontroller, for each   board you selected when creating the project. 
\end{itemize}
...
\begin{itemize}
\item
  Right-click the \path{src} folder in the \inline{Project} tab, and select   \inline{New File} from the context menu that appears.
\item
  Enter the file name when prompted, I chose \path{AVRBlink.c}, but you can name   the file as you wish.
\end{itemize}
...
This is because \indexasis{PlatformIO} knows that an Arduino Nano with an \indexasis{ATmega328P} runs with a 16MHz crystal, so it defines the correct speed for us. This is defined in the \path{/home/norman/.platformio/packages/framework-arduinoavr/boards.txt} file.
...
Looking at the last few lines of the compiler output text for the \path{AVRBlink.c} example, we see the following, which you may remember from above:
...
By default, \indexasis{PlatformIO} assumes that you will be using a USB cable, so the file \path{platformio.ini} is already suitable, and when you come to upload, the desired port will (or should) be detected automatically.

If you are using an \indexasis{ISP Programmer} instead of the USB cable, then you must update the \path{platformio.ini} file to tell it which programmer you are using and the protocol. In addition, to upload firmware using an ISP programmer, you need to use \inline{target = program} instead of \inline{target = upload} - which is the default. 

You will need to add \inline{target = program} to your \path{platformio.ini} file. Don't forget to save it before attempting to upload/program!\footnote{Yes, I forgot!} When you next attempt an upload, you will see the text \inline{target: program} at the very top of the compile/upload listing when you press F8.


Here are a few configurations for various programmers, taken from the \href{http://docs.platformio.org/en/latest/platforms/atmelavr.html?highlight=usbtiny#upload-using-programmer}{PlatformIO docs}. Add the following lines to your \path{platformio.ini} file for the corresponding programmer:
...
\begin{lstlisting}[numbers={none},caption={The \inline{platformio.ini} additions for `USBasp' ISP Programmer}]
...
\begin{itemize}
\item Connect your \indexasis{ISP Programmer} cable to the \indexasis{Arduino} and to your computer.
...
\begin{note}
	If you ever find a bargain AVR micro controller on Ebay\footnote{Other online auction sites are available!} which doesn't have an Arduino bootloader burned in already, then you will have to use either an \indexasis{ISP Programmer} or an existing \indexasis{Arduino} to do the above \emph{before} the chip can be programmed using the USB cable in the normal \indexasis{Arduino} manner.
\end{note}
\end{warning}
