...
Hopefully, that will all work, however, as it uses the \inline{sudo} command, it \emph{may} fail to execute properly if your version of \inline{sudo} requires a password. However, PlatformIO helpfully displays the two commands that need to be executed in a shell session to enable the shell commands:
...
\emph{Obviously} you will have to replace <your name> as appropriate, but PlatformIO will display the correct commands for you to type in. When you do enter them, you will have to supply your \inline{sudo} password.
...
Open the Atom editor, if not already open, and click \inline{PlatformIO$\rightarrow$Project Examples}. You will be notified, on the first occasion, that internet access will be required.
...
\lstinputlisting[language=C,caption={Arduino Blink Example}]{AVRCode/Part1/PlatformIO_Quick_Start/ArduinoBlink.cpp}
...
Open the Atom editor, if not already running, and make sure that you can see the PlatformIO Home Screen. Click on \inline{+ New Project} to start the New Project Wizard. Alternatively, click \inline{PlatformIO$\rightarrow$Initialize or Update PlatformIO Project}.
...
\lstinputlisting[language=C,caption={AVRBlink.c - AVR Blink Example}]{AVRCode/Part1/PlatformIO_Quick_Start/AVRBlink.c}
